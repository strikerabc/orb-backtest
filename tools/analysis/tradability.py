...
# Required because this file lives in a tools/ subdirectory: running it as a
# script puts its OWN directory on sys.path[0], not the repo root, so
# `from src...` fails. It worked before only because the file sat at the repo
# root, where sys.path[0] happened to be the root.
import sys
