...
def _eligible_end(data_end: date) -> date:
    ...
    Must honour HOLDOUT_PIN_START. On this branch the pin is set, so the boundary
    is fixed rather than sliding with data_end; asserting against the sliding value
    would compare windows to a boundary the code never used.
    """
    if HOLDOUT_PIN_START is not None:
        return pd.Timestamp(HOLDOUT_PIN_START).date()
    return pd.Timestamp(data_end - pd.DateOffset(months=HOLDOUT_MONTHS)).date()


def _sliding_eligible_end(data_end: date) -> date:
    """The unpinned boundary, for reproducing the pre-fix (pre-pin) behaviour."""
    return pd.Timestamp(data_end - pd.DateOffset(months=HOLDOUT_MONTHS)).date()
...
def _legacy_select(data_start: date, data_end: date):
    ...
    # The pre-fix code predates the pin, so it must reproduce against the SLIDING
    # cutoff regardless of what HOLDOUT_PIN_START is set to now.
    eligible_end = _sliding_eligible_end(data_end)
...
def test_legacy_logic_did_breach_the_holdout():
    ...
    """
    eligible_end = _sliding_eligible_end(ETH_DATA_END)
